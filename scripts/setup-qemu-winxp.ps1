...
Write-AsciiFile $runCmd (@(
    ...
    "rem  - $($XhciDevice): no in-box XP driver for PCI\CC_0C0330, so it shows as an",
    "rem    unrecognised ""Universal Serial Bus (USB) Controller"" and the package",
    "rem    installs through the INF's .NTx86 half from the transfer drive.",
    ...
    "rem (no port=; qemu-xhci refuses a port number and picks a USB2 port).",
